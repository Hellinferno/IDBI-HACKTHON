...
## Principles
1. **Deterministic** â€” same inputs â†’ byte-identical output. No randomness, no time-of-day drift (time passed explicitly).
2. **Pure functions** â€” no DB, no network inside math; inputs in, result out.
3. **Decimal-safe** â€” use `Decimal`; never float for money.
4. **Explainable** â€” every result carries the inputs and intermediate steps for the "why" card.
5. **Versioned assumptions** â€” return assumptions used (inflation, return rates) with every result.
...
- formula: FV = P Ã— [((1+i)^n âˆ’ 1)/i] Ã— (1+i), i = monthly rate.
...
- triggers only when |actual âˆ’ target| > threshold (default 5%).
...
- Property tests: monotonicity (more contribution â‡’ â‰¥ corpus), boundary (zero return, zero months).
