...
Wealth advisory is fragmented and inaccessible to most retail customers. Banks hold rich data — transactions, balances, income flow, holdings — but rarely turn it into timely, personalized guidance. Human advisors do not scale to millions of customers and ignore the long tail (mass-affluent and below).
...
- Surfaces guidance through a **3D avatar** — a geometric/orb object, not a human face — that speaks via TTS and reacts visually to its own audio and the user's financial state.
...
|---|---|---|
| Lip-sync | Required, hard, expensive | **Not needed** — orb pulses to audio amplitude |
...
**The Orb expresses state through 4 channels:** color (mood/market), motion (calm ↔ agitated), scale/pulse (audio-reactive while speaking), and form/particles (idle, listening, thinking, alerting, celebrating).

## 4. Core capabilities

1. **Behavioral profiling** — auto-categorize transactions; derive spend personality, risk score, savings rate, life stage.
2. **Goal planning** — retirement, home, education, emergency fund; corpus + gap computed deterministically.
3. **Recommendation** — products, SIP amounts, asset allocation, idle-cash sweep, tax optimization (80C/ELSS).
4. **Conversational advisor** — RAG-grounded chat/voice, plain-language explanations, vernacular support.
5. **Proactive nudges** — idle cash, missed SIP, overspend, goal drift, tax-deadline.
6. **Compliance** — suitability checks, mandatory disclaimers, full audit trail of every recommendation.
