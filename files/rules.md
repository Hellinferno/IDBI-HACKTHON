...
3. Engine functions are pure and deterministic. Same inputs → identical output. Time enters only via an explicit `asOf` parameter.
4. Every number shown must be explainable — the engine returns `steps`; the UI exposes a "why".
...
## Avatar
11. The avatar is a **3D object, not a face.** No facial features, no lip-sync, no visemes — anywhere in the system.
...
## Conversation
29. If information is outside the RAG corpus or user data, the advisor says so — it does not guess.
