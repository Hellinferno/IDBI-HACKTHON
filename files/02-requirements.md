...
- FR-3.3 What-if simulation (change horizon/amount/contribution → live recompute).
...
- FR-6.2 Map advisor states → visual states (idle, listening, thinking, speaking, alert, celebrate).
...
| NFR-3 | Avatar render | ≥ 30 fps mid-tier Android |
...
| NFR-8 | Audit retention | per regulator (≥ 7 yrs) |
| NFR-9 | Determinism | identical inputs → identical engine output |
